...
def run_team_analysis_dashboard(load_data_func, data_file):
    ...
    # --- Load Data ---
    try:
        # The first argument (load_data_func) is called with the second argument (data_file)
    ...
    # Check if the dataframe contains the required columns before filtering
    if 'season' not in df.columns or 'teamname' not in df.columns:
        st.warning("Data is missing 'season' or 'teamname' columns. Cannot display analysis.")
        return
    ...
    # Ensure 'winner' column exists for grouping
    if 'winner' in df_filtered.columns:
        team_wins = df_filtered.groupby("teamname")["winner"].sum().sort_values(ascending=False)
        if not team_wins.empty and team_wins.sum() > 0:
            ...
            st.info("No wins recorded for selected teams in this season.")
    else:
        st.info("Required 'winner' column not found in data.")
    ...
    if 'points' in df_filtered.columns:
        ...
        st.info("Required 'points' column not found in data.")
    ...
    if 'position' in df_filtered.columns:
        ...
        st.info("Required 'position' column not found in data.")
